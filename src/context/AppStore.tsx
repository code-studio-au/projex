...
type StoreState = {
  ...
  setProjectData: (
    projectId: ProjectId,
    patch: Partial<ProjectDataSlice>
  ) => void;
  ...
  getPrimaryCompanyForUser: (
    userId: UserId
  ) => { companyId: CompanyId; role: CompanyRole } | null;
...
export function AppStoreProvider(props: { children: React.ReactNode }) {
    ...
    () =>
      persisted?.users ?? [
        ...
        },
        {
          id: asUserId('u_lead'),
          email: 'lead@acme.co',
          name: 'Priya Project Lead',
        },
        {
          id: asUserId('u_member'),
          email: 'member@acme.co',
          name: 'Theo Team Member',
        },
        {
          id: asUserId('u_viewer'),
  ...
  const getPrimaryCompanyForUser = (userId: UserId) =>
    getPrimaryCompanyForUserFn(userId, companyMemberships);

  // selection: default to primary company of current user (or first company)

  const getUserCompanyId = (userId: UserId): CompanyId | null =>
    getPrimaryCompanyForUser(userId)?.companyId ?? null;
  const getUserCompanyRole = (userId: UserId): CompanyRole | null =>
    getPrimaryCompanyForUser(userId)?.role ?? null;

  const getUserCompanyRoles = (userId: UserId): CompanyRole[] => {
    const cid = getUserCompanyId(userId);
    if (!cid) return [];
    return companyMemberships
      .filter((m) => m.userId === userId && m.companyId === cid)
      .map((m) => m.role);
  };
  ...
  const defaultCompanyId = useMemo(() => {
    return (
      getPrimaryCompanyForUser(currentUserId)?.companyId ?? companies[0].id
    );
  }, [currentUserId, companies, companyMemberships]);

  const [activeCompanyId, setActiveCompanyId] =
    useState<CompanyId>(defaultCompanyId);
  ...
  // project data
  const [dataByProjectId, setDataByProjectId] = useState<
    Record<ProjectId, ProjectDataSlice>
  >(() => {
  ...
  const removeCompany = (companyId: CompanyId) => {
    ...
    setCompanyMemberships((prev) =>
      prev.filter((m) => m.companyId !== companyId)
    );
    ...
    setDataByProjectId((prev) => {
      const next = { ...prev };
      for (const p of projects.filter((p) => p.companyId === companyId))
        delete next[p.id];
      return next;
    });
    ...
    if (activeCompanyId === companyId) {
      const fallback =
        companies.find((c) => c.id !== companyId)?.id ?? 'co_acme';
  ...
  const upsertCompanyMembership = (
    ...
    setCompanyMemberships((prev) => {
      ...
      const exists = kept.some(
        (m) =>
          m.companyId === companyId && m.userId === userId && m.role === role
      );
