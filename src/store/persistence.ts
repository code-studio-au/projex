...
export function loadPersistedState(): PersistedStateV1 | null {
  try {
    ...
    const companyMemberships = Array.isArray(parsed.companyMemberships)
      ? parsed.companyMemberships
      : null;
    const projectMemberships = Array.isArray(parsed.projectMemberships)
      ? parsed.projectMemberships
      : null;
    ...
    const companyId =
      (parsed.activeCompanyId as CompanyId) ?? seedState.activeCompanyId;
    ...
    const projectExists = projectId
      ? projects.some(
          (p) => p.id === projectId && p.companyId === safeCompanyId
        )
      : false;
