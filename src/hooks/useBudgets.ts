...
import type { BudgetLine, BudgetLineId, CategoryId, CompanyId, ProjectId, SubCategoryId } from "../types";
import { asBudgetLineId, asCompanyId, asProjectId } from "../types";
import { uid } from "../utils/id";

export function useBudgets(params: { companyId?: CompanyId; projectId?: ProjectId; initial?: BudgetLine[]; value?: BudgetLine[]; onChange?: (next: BudgetLine[]) => void }) {
  ...
  const upsertBudgetForSubCategory = (subCategoryId: SubCategoryId, categoryId: CategoryId) => {
    setBudgets((prev) => {
      ...
      return [...prev, { id: asBudgetLineId(uid("bud")), companyId: params.companyId ?? asCompanyId("co_unknown"), projectId: params.projectId ?? asProjectId("prj_unknown"), categoryId, subCategoryId, allocated: 0 }];
    });
  };
