...
import { requireUserId, toServerSession, type ServerSession } from '../auth/session';
...
export type ServerFnContextInput = {
   ...
   */
  auth?: { userId?: string | null; user?: { id?: string | null } | null } | null;
  /**
...
async function resolveSession(context: ServerFnContextInput): Promise<ServerSession | null> {
...
export async function requireServerUserId(context: ServerFnContextInput): Promise<UserId> {
...
export function assertContextProvided(context: ServerFnContextInput | undefined): asserts context is ServerFnContextInput {
  if (!context) {
