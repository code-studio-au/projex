...
export async function resetDatabaseToSeed(): Promise<void> {
  ...
  await db.transaction().execute(async (trx) => {
    ...
    await trx.insertInto('company_memberships').values(seed.companyMemberships.map((m) => ({
      company_id: m.companyId,
      user_id: m.userId,
      role: m.role,
    }))).execute();

    await trx.insertInto('project_memberships').values(seed.projectMemberships.map((m) => ({
