...
export function normalizeExternalId(value: string | null | undefined): string | undefined {
  const next = value?.trim();
  return next ? next : undefined;
}

export function normalizeTxnPatch(input: TxnUpdateInput): Partial<Txn> & { id: TxnId } {
...
function compareOptionalDecimalString(a: string | undefined, b: string | undefined): number {
  if (a && b) {
...
export function sortTransactionsForList<T extends Pick<Txn, 'createdAt' | 'internalId'>>(
